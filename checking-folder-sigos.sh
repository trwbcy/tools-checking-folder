...
data_folder="/data/tsel/g_xc/airflow/script/rpa_probing/dataset/umb_sigos"
date_folder="${1}"
...
function display_missing_files() {
  ...
  if [[ ${#missing_files[@]} -eq 0 ]]; then
    echo -e "${HIGHLIGHT}Semua file lengkap dalam ${data_folder}/${date_folder}${NC}"
  else
    echo -e "${GREEN}Terdapat file missing di dalam ${data_folder}/${date_folder}:${NC}"
    echo -e "${GREEN}==============================================${NC}"
...
if [[ -d "${data_folder}/${date_folder}" ]]; then
  display_folder_content "${data_folder}/${date_folder}"
  missing_files1=()
  for file in "${required_files[@]}"; do
    # Menggunakan find dengan opsi -iname untuk pencarian tidak sensitif huruf besar/kecil
    if ! find "${data_folder}/${date_folder}" -iname "*${file}*" >/dev/null 2>&1; then
      missing_files1+=("${file}")
    fi
  done
  display_missing_files "${missing_files1[@]}"
else
  echo -e "${GREEN}Folder ${data_folder}/${date_folder} tidak ditemukan.${NC}"
fi
